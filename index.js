...
async function startBot() {
  ...
  // Check if sessionID is provided and process KnightBot! or TayyabBot! format session
  if (config.sessionID && (config.sessionID.startsWith('KnightBot!') || config.sessionID.startsWith('TayyabBot!'))) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if ((header !== 'KnightBot' && header !== 'TayyabBot') || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'TayyabBot!.....' or 'KnightBot!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from Tayyab Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing Tayyab session:', e.message);
